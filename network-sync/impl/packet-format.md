...
整包长度必须等于 `24 + payload_len`，且不超过 `MaxPacketBytes=1200`。发送前超限则应用层拆片；解码时对不上则丢弃。

## 整包

```text
偏移    0        4    5    6        8        12       16       20       24
        +--------+----+----+--------+--------+--------+--------+--------+-------
字节    | magic  |type|rsv | plen   |session | seq    | ack    |ackbits | payload
宽度    |   4    | 1  | 1  |   2    |   4    |   4    |   4    |   4    | plen
```
...
| 12 | 4 | u32 | seq | 当前 session 内递增，从 1 起 |
| 16 | 4 | u32 | ack | 本端作为接收方、对**当前对端**已收到的最大 seq |
| 20 | 4 | u32 | ack_bits | 选择性确认。见下方序号窗 |
| 24 | plen | — | payload | 由 type 决定 |

不进字节的字段：`Src`、`Dst`、`DeliverAt`。由 UDP 地址表或假网络邮箱提供。

`session` / `seq` / `ack` / `ack_bits` 按 **(接收端, 发送端)** 记账。两个客户端各自从 1 编号，发往服务器时互不覆盖。
发送端重启时换 session 并把 seq 置回 1；接收端切换 epoch 后拒绝退役 session 的迟到包。
...
超长内容在应用层拆成多个完整 Ns 包（各带 24 字节头），互不依赖。丢其中一个不影响其它。禁止依赖内核 IP 分片。

| 包 | 典型整包 | 单数据报最多条目 |
| --- | --- | --- |
| `C2SInput` 锁步 | 27 B | 234 |
| `S2CFrame` 4 拍 | 53 B | 195 |
| `S2CSnapshot` | 45 B | — |
| `P2PInput` 4 条 | 45 B | 235 |
...
| 9 | `C2SFrameNack` | 客→服 | 乐观锁步按号补发 | 2 + 4×count |
...
payload 长度：`3 + 5×win`。乐观锁步整包 27 字节。等齐 `win=1` 整包 32 字节。状态同步 `win≤8` 时整包最多 67 字节。

锁步示例（player=0，dx=+1，win=0，session=`0x12345678`，seq=9，ack=4，ack_bits=0）：

```text
53 4E 41 54  01 00  03 00  78 56 34 12  09 00 00 00
04 00 00 00  00 00 00 00
00 01 00
```

状态同步保留最多 `MaxInboxAhead` 个未确认输入，每包从最老缺口起重发最多 8 个 `(seq, dx)`。
...
payload 长度：`5 + 6×count`。count=4 时整包 53 字节。

通信回合把 `Tick` 设为 2–6（下一回合 `FramesPerTurn`）。此时每条多 1 字节 `u8`
该回合长度（2–6），payload 为 `5 + 7×count`。reserved 为 `(ClosedLen<<4)|NextFpt`；
只带 NextFpt 的旧包（reserved 2–6）仍解码。`BaseTick` 在内存里是 ClosedLen。
...
payload 5 字节。整包 29 字节。客户端收到可用快照后连发两次，抗丢包。
...
payload 长度：`1 + 5×count`。count=4 时整包 45 字节。
...
payload 9 字节。整包 33 字节。锁步客户端每 15 拍发一次。服务器对照自己的 `Checksums[frame]`：相同则 `ChecksumOk++`，不同则 `bDesync`。未知拍号忽略。
...
payload 长度：4。整包 28 字节。无 ACK。实现里服务器每个锁步加门泵周期重发当前值。

## 9 `C2SFrameNack`

乐观锁步客户端点名缺拍。`SeqWindow` 是缺的逻辑拍号，不是状态同步的输入 seq。等齐 / 通信回合 / delay 第一版不发这个 type。

| 偏移 | 宽 | 类型 | 字段 | 约束 |
| --- | --- | --- | --- | --- |
| 0 | 1 | u8 | player_id | 0 或 1。结算用 UDP 源地址 |
| 1 | 1 | u8 | count | 后面拍号数。`>255` 失败。实现里最多 `LockstepNackMax=8` |
| 2 | 4×count | u32 | frames | 缺的 `ExecFrame` 起连续空洞 |
