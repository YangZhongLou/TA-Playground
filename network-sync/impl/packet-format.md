...
| 4 | 1 | u8 | type | `ENsMsg` 1–7。其它值丢弃 |
| 5 | 1 | u8 | reserved | 编码器写 `0`。解码读掉，不解释 |
...
| 7 | `S2CJoinSnap` | 服→客 | 锁步重连 | 17 + 6×count |
...
| 2 | 1 | u8 | win | 后面条目数。`>255` 编码失败。锁步为 0 |
...
payload 长度：`3 + 5×win`。锁步整包 23 字节。状态同步 `win≤8` 时整包最多 63 字节。
