...
一支一个目录文件、一套类型、一个测试前缀。不要 `NsHybrid.cpp` 用枚举切四支。
...
停拍后不调用 `Tick`，直到两槽对 `LiveSnap` checksum ack。乐观 `ANsNetManager` 走停拍拉齐泵（假网络 / LocalMesh / Host/Client），并在同一轮 Drain 叠 `S2CDoorOpen`。锁步加门不新增 `ENsScheme`。
