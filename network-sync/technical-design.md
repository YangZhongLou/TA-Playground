...
| 锁步 | `Lockstep.*` | 乐观：干净、Drop、Join、空洞、分叉。等齐：`Lockstep.Wait.*`（含 Join、停拍拉齐）。通信回合：`Lockstep.Turn.*`（含 Speed / Recovery）。delay：`Lockstep.Delay.*`（含停拍拉齐） |
| 结合 | `Lockstep.Resync.*` / `Lockstep.Wait.Resync.*` / `Lockstep.Delay.Resync.*` / `LockstepDoor.*` | 乐观、等齐、delay 停拍强制回跳与恢复（含包驱动 Host/Client）；假网络乐观演示走同一泵并叠门；FakeNet 门；检查点用 `Lockstep.Join*`；切段 `SchemeSwitch` / `SchemeApply` |
...
| 未做 | 影响 |
| --- | --- |
| 停拍拉齐 | 乐观、等齐、delay Manager 走各自停拍泵；Host/Client 靠 LiveSnap 包。通信回合尚未叠 |
...
| [impl/hybrid/README.md](impl/hybrid/README.md) | 结合包：检查点 / 切段 / 停拍拉齐 / 等齐与 delay 停拍拉齐 / 锁步加门 |
